...
breakOn
  (AT nba@(ByteArray nba#) noff@(I# noff#) nlen@(I# nlen#))
  haystack@(AT hba@(ByteArray hba#) hoff@(I# hoff#) hlen@(I# hlen#))
    ...
    where
      ...
      go
        ...
        | otherwise = I# (cFindFirstBlock nba# noff# nlen# hba# hoff# hlen#)
      w8 :: Word8
      ...
      | otherwise =
        let final = computeMulaMatch i
         ...
         in if final == zeroBits
              then goBig acc $ i + off + indexSmallArray lastOcc last
...
foreign import ccall unsafe "find_first_block"
  cFindFirstBlock :: ByteArray# -> Int# -> Int# -> ByteArray# -> Int# -> Int# -> Int#
